...
The root provider for your Design System components. An abstraction around `styled-component`'s `ThemeProvider` that also
includes `GlobalStyles` as well as an unique provider supplying values to components.
