...
A `DatePicker` is used to select a date. The input is modelled around the [`Combobox`](https://github.com/strapi/design-system/tree/main/packages/strapi-design-system/src/Combobox) ARIA pattern with a `dialog`
...
By default, the DatePicker is localised to the user's browser locale whilst the calendar's initial date is set to `today`.
...
DatePicker wrapped with [`Field`](https://github.com/strapi/design-system/tree/main/packages/strapi-design-system/src/Field) component to create a powerful complete form input
