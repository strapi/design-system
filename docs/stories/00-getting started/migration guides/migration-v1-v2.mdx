...
The following components have been removed from DS and moved to CMS where it belongs. When working on the Strapi CMS (creating a plugin, for example), you can still use these components by importing them from `@strapi/strapi/admin`.
