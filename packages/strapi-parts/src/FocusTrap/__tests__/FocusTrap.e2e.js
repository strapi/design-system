...
test.describe('FocusTrap', () => {
  ...
  test.describe('Pressing Tab in the trap', () => {
    ...
    test('traps the focus when pressing Tab for Webkit', async ({ page, browserName }) => {
      test.skip(['firefox', 'chromium'].includes(browserName));
    ...
    test('traps the focus when dynamically adding an element in the focus tree and pressing Tab for Webkit', async ({
      page,
      browserName,
    }) => {
      test.skip(['firefox', 'chromium'].includes(browserName));
