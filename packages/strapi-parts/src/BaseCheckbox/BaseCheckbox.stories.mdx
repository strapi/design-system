...
<Meta title="Design System/Atoms/BaseCheckbox" component={BaseCheckbox} />
...
The checkbox can have an indeterminate state.
    ...
    {() => {
      return (
        <>
          ...
          <BaseCheckbox name="disabled-checked" disabled value={true} aria-label="Last checkbox" />
        </>
      );
    }}
  </Story>
...
<Canvas>
  <Story name="L Size">
    {() => {
      const [val, setValue] = useState(false);
      return (
        <BaseCheckbox name="LSize" size="L" onValueChange={(value) => setValue(value)} value={val} aria-label="Lsize" />
      );
    }}
  </Story>
</Canvas>
