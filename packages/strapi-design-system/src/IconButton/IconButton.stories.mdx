...
import Ellipsis from '@strapi/icons/Ellipsis';
    ...
    {() => {
      return (
        <Stack spacing={1} horizontal>
          <IconButton onClick={() => {}} label="..." icon={<Ellipsis />} />
          <Button variant="default" disabled>Save as draft</Button>
          <Button variant="default" disabled>Save & Publish</Button>
        </Stack>
      );
    }}
  </Story>
