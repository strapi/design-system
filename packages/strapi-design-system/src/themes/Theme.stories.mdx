...
<Canvas>
  <Story name="light colors">
    {() => {
      ...
      const colorsKey = Object.keys(theme.colors).filter((color) => !color.includes('warning') && !color.includes('button'));
      const colors = [];
                ...
                <Stack size={2}>
                  {colorGroup.map((color) => (
                    <Box
                      ...
                      padding={3}
                      background={color}
                      color={conditions.some((v) => color.includes(v)) ? 'neutral0' : 'neutral900'}
                    >
                      <Typography 
                        textColor={conditions.some(v => color.includes(v)) ? 'neutral0' : 'neutral800'}
                      >
                        {color}
                      </Typography>
                    </Box>
              ...
              <GridItem key={`color-group-${idx}`} padding={4}>
                <Stack size={2}>
                  {colorGroup.map(({ colorKey, colorValue }) => (
                    ...
                    >
                      <Typography 
                        textColor={conditions.some(v => colorKey.includes(v)) ? 'neutral800' : 'neutral0'}
                      >
                        {colorKey}
                      </Typography>
                    </div>
