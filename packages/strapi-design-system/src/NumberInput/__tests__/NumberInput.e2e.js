...
test.describe.parallel('NumberInput', () => {
  test.describe('light mode', () => {
    ...
    test.describe('with initial 0 value', () => {
      test.beforeEach(async ({ page }) => {
        // This is the URL of the Storybook Iframe
    ...
    test.describe('with initial undefined value', () => {
      test.beforeEach(async ({ page }) => {
        // This is the URL of the Storybook Iframe
        await page.goto('/iframe.html?id=design-system-components-numberinput--with-initial-undefined&viewMode=story');
        await injectAxe(page);
      });

      test('shows an initial value of empty string (default value)', async ({ page }) => {
        const value = await page.$eval('input', (el) => el.value);
        expect(value).toBe('');
      });
    });
